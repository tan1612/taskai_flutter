...
class _TimetableGridViewState extends ConsumerState<TimetableGridView> {
  ...
  void _handleTransformChanged() {
    final double scale = _transformationController.value.entry(0, 0);
  ...
  void _zoomIn() {
    final double targetScale = (_currentScale + 0.15).clamp(0.2, 3.0);
  ...
  void _updateScale(double targetScale) {
    ...
    final double translationX = currentMatrix.entry(0, 3);
    final double translationY = currentMatrix.entry(1, 3);

    final translation = Matrix4.translationValues(translationX, translationY, 0.0);
    final scale = Matrix4.diagonal3Values(targetScale, targetScale, 1.0);
    final newMatrix = translation * scale;
  ...
  Widget build(BuildContext context) {
    ...
    // Automatically fit the timetable width on first build
    if (!_isScaleInitialized) {
      final screenWidth = MediaQuery.of(context).size.width;
      final double fitScale = (screenWidth - 16.0) / totalWidth;
      final double initialScale = fitScale.clamp(0.2, 3.0);
    ...
    return Stack(
      children: [
        ...
        // Bộ nút Zoom (+) (-) nổi trên góc trái dưới cùng
        Positioned(
          left: 16,
          bottom: 16,
          child: Card(
